# spark-app/main_optimized.py
# PySpark пайплайн для анализа поведения кредитных транзакций (оптимизированная версия)
# Включает: repartition(), cache()/persist(), минимизация shuffle операций, улучшенный параллелизм
...
logger = setup_logging('CreditCardPipeline-Optimized')

def preprocess_data(df, optimal_partitions, logger):
    """Создание временных признаков с оптимизацией через перераспределение партиций"""
    logger.info("Начало предобработки данных с оптимизацией...")
    
    try:
        # Репартиционирование для оптимальной параллелизации
        ...
        # Кэшируем промежуточный результат (будет использован дальше)
        df.cache()
        df.count()  # Принудительное кэширование
        logger.info("Предобработка завершена и закэширована")
        return df
...
def build_user_profiles(df, logger):
    """Построение профилей пользователей с оптимизацией"""
    logger.info("Начало построения пользовательских профилей (оптимизированно)...")
    
    try:
        # Используем более эффективное агрегирование
        ...
        # Пиковый час - использует window function (оптимальнее чем RDD)
        ...
        # Кэшируем для дальнейшего использования в identify_interesting_users
        user_profiles.cache()
        user_count = user_profiles.count()
        logger.info(f"Профили построены для {user_count:,} пользователей (закэшированы)")
...
def identify_interesting_users(user_profiles, df, logger):
    """Поиск аномальных пользователей с оптимизациями"""
    logger.info("Начало выявления интересных пользователей (оптимизированно)...")
...
def main():
    ...
    logger.info("Запуск Spark-приложения (оптимизированная версия)")
    ...
    try:
        ...
        if num_datanodes >= 3:
            ...
            logger.info(f"Конфигурация для {num_datanodes} узлов (максимальная оптимизация)")
        else:
            logger.info("Конфигурация для 1 узла (стандартная оптимизация)")
        
        spark = config_builder.getOrCreate()
        logger.info("Spark инициализирована (с оптимизацией)")

        logger.info(f"Оптимальное число партиций: {optimal_partitions} (DataNodes: {num_datanodes})")
        ...
        # Стадия 2: Предобработка данных (с оптимизацией)
        stage_timer.start_stage("Предобработка данных (с оптимизацией)")
        df = preprocess_data(df, optimal_partitions, logger)
        stage_timer.end_stage()
        ...
        exp_name = os.getenv('EXP_NAME', 'exp_1dn_optimized')
        stages = stage_timer.get_stages()
        save_metrics(spark, df, elapsed, True, logger, exp_name, stages)
        ...
        try:
            ...
            pass
        logger.info("Кэш очищен, приложение завершено успешно")
